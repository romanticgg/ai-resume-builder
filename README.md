...
这个系统的作用就是： 把用户输入的原始信息，整理并优化成一份更专业的简历内容。该项目是基于人工智能的求职辅助系统的一部分。
...
├── backend/
│   ├── app.py              # Flask 后端入口
│   └── formatter.py        # 简历生成与优化逻辑
├── frontend/
│   └── index.html          # 前端页面（表单 + 简历展示 + PDF导出）
├── screenshots/            # README中用到的截图
│   ├── form.png
│   ├── result.png
│   └── optimization.png
├── data/                   # 预留数据目录（用于后续扩展,如存储用户简历、模板等）
├── requirements.txt
└── README.md
```
